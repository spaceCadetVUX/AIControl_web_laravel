...
(function ($) {
	...
	// 41. Fade Effect With Scroll  (safer)//
	...
	document.querySelectorAll(".tp_reveal_anim").forEach(el => observer.observe(el));
	});
	...
	// 42. project anim  (safer)//
	if (document.querySelector('.design-project-item')) {
	const pw = gsap.matchMedia();

	pw.add("(min-width: 991px)", () => {
		// Set smaller initial offset, no rotation
		gsap.set('.design-project-thumb.item-1', { x: 100, opacity: 0.8 });
		gsap.set('.design-project-thumb.item-2', { x: -100, opacity: 0.8 });

		// Animate only once when element enters viewport (no scrub)
		document.querySelectorAll('.design-project-item').forEach(item => {
		gsap.to(item.querySelectorAll('.design-project-thumb'), {
			x: 0,
			opacity: 1,
			duration: 1.2,
			ease: "power2.out",
			...
			trigger: item,
			start: "top 85%",
			toggleActions: "play none none reverse", // no continuous scrub
			}
		});
		});
	});
   ...
   // 76. perspective-slider animation //
	...
	// 93. Smooth scroll to section  (click to scroll) //
